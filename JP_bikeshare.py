...
def get_filters():
    ...
    print('Please note that there are data for only 3 cities: Chicago, New york city and Washington')
    ...
    y=True
    while y is True:
        city_input=(input('Please select the city that you would like to inquire about')).lower()
        if city_input in cities:
            city=city_input
            break
        else:    
            print('Sorry, your City input is not within the three. Kindly input chicago,new york city or washington')
            y=True
        
    while y is True:   
        month_input=(input('Please select the month that you would like to filter with')).lower()
        if month_input in correct_months:
            month=month_input
            break
        else:    
            print('Sorry, your Month input is not valid.')
            y=True
    while y is True:
        day_input=(input('Please select the day that you would like to filter with')).lower()
...
def load_data(city, month, day):
    ...
    # This to enable loading of data file into a dataframe
    df = pd.read_csv(CITY_DATA[city])

    # To be able to convert the Start Time column to datetime
    df['Start Time'] = pd.to_datetime(df['Start Time'])

    # To extract month and day of week from Start Time and then create new columns
    df['month'] = df['Start Time'].dt.month
    df['day_of_week'] = df['Start Time'].dt.day_name()
    # To get hour from Start Time to create hour column
    df['hour'] =df['Start Time'].dt.hour

     # To filter by month if applicable
    if month != 'all':

        # To use the index of the months list to get the corresponding int
